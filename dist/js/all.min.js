...
(function() {
  ...
  Caruso = (function() {
    ...
    function Caruso(elem1) {
      var container, elem, left, right, scrollable, thiss;
      this.elem = elem1;
      elem = this.elem;
      ...
      if (scrollable.length) {
        scrollable[0].addEventListener('scroll', function(e) {
          ...
          if ((scrollable[0].scrollLeft + scrollable[0].offsetWidth) >= container[0].offsetWidth) {
            return Caruso.hide(right[0]);
          } else {
            return Caruso.show(right[0]);
          }
        });
        if (left.length) {
    ...
    Caruso.prototype.moreItems = function() {
      var container, elem, right, thiss;
      elem = this.elem;
      right = elem.querySelectorAll('.right');
      container = elem.querySelectorAll('.horizontal');
      console.log('flagAjax:', this.flagAjaxFree);
      thiss = this;
      if (container.length) {
        if (this.flagAjaxFree) {
          this.flagAjaxFree = false;
          return Ajax.getCb('/xhr-html.html?' + Math.random(), function(xhr) {
    ...
    Caruso.prototype.fixLengths = function(incr) {
